...
{
	void ExVertex::exportGeometryXml(tinyxml2::XMLElement* vertexbufferElem, tinyxml2::XMLDocument* pXMLDocument)
	{
		tinyxml2::XMLElement* vertexElem = pXMLDocument->NewElement("vertex");
		vertexbufferElem->InsertEndChild(vertexElem);

		tinyxml2::XMLElement* positionElem = pXMLDocument->NewElement("position");
		vertexElem->InsertEndChild(positionElem);
		positionElem->SetAttribute("x", vPos.x);
		positionElem->SetAttribute("y", vPos.y);
		positionElem->SetAttribute("z", vPos.z);

		tinyxml2::XMLElement* normalElem = pXMLDocument->NewElement("normal");
		vertexElem->InsertEndChild(normalElem);
		normalElem->SetAttribute("x", vNorm.x);
		normalElem->SetAttribute("y", vNorm.y);
		normalElem->SetAttribute("z", vNorm.z);
	}

	void ExVertex::exportTextureCoordXml(tinyxml2::XMLElement* vertexbufferElem, tinyxml2::XMLDocument* pXMLDocument)
	{
		tinyxml2::XMLElement* vertexElem = pXMLDocument->NewElement("vertex");
		vertexbufferElem->InsertEndChild(vertexElem);

		tinyxml2::XMLElement* texcoordElem = pXMLDocument->NewElement("texcoord");
		...
		{
			texcoordElem->SetAttribute("u", 0);
			texcoordElem->SetAttribute("v", 1);
		}
	}

	void ExVertex::exportVertexBoneAssignmentXml(tinyxml2::XMLElement* vertexboneassignmentElem, tinyxml2::XMLDocument* pXMLDocument)
	{
		if (lWeight.size() > 0 && lBoneIndex.size() > 0)
		{
			vertexboneassignmentElem->SetAttribute("boneindex", lWeight[0]);
			vertexboneassignmentElem->SetAttribute("weight", lBoneIndex[0]);
		}
	}

	void ExFace::exportFaceXml(tinyxml2::XMLElement* facesElem, tinyxml2::XMLDocument* pXMLDocument)
	{
		tinyxml2::XMLElement* faceElem = pXMLDocument->NewElement("faces");
		faceElem->InsertEndChild(faceElem);

		if (vertices.size() == 3)		// 这个面不是 3 个顶点
		{
			faceElem->SetAttribute("v1", vertices[0]);
			faceElem->SetAttribute("v2", vertices[1]);
			faceElem->SetAttribute("v3", vertices[2]);
		}
		else	// 一个面不是 3 个顶点，如何处理
		{
			faceElem->SetAttribute("v1", 0);
			faceElem->SetAttribute("v2", 0);
			faceElem->SetAttribute("v3", 0);
		}
	}

	void ExSubMesh::export_Face_Geometry_BoneAssignments_Xml(tinyxml2::XMLElement* submeshesElem, tinyxml2::XMLDocument* pXMLDocument)
	{
		tinyxml2::XMLElement* submeshElem = pXMLDocument->NewElement("submesh");
		submeshesElem->InsertEndChild(submeshElem);

		// 导出面
		tinyxml2::XMLElement* facesElem = pXMLDocument->NewElement("faces");
		submeshElem->InsertEndChild(facesElem);

		facesElem->SetAttribute("count", (int)m_faces.size());

		std::vector<ExFace>::iterator faceBeginIte, faceEndIte;
		faceBeginIte = m_faces.begin();
		faceEndIte = m_faces.end();
		for (; faceBeginIte != faceEndIte; ++faceBeginIte)
		{
			faceBeginIte->exportFaceXml(facesElem, pXMLDocument);
		}

		// 导出 geometry
		tinyxml2::XMLElement* geometryElem = pXMLDocument->NewElement("geometry");
		submeshElem->InsertEndChild(geometryElem);
		geometryElem->SetAttribute("geometry", (int)m_vertices.size());

		// 导出 positions 和 normals
		std::vector<ExVertex>::iterator vertexBeginIte, vertexEndite;
		vertexBeginIte = m_vertices.begin();
		vertexEndite = m_vertices.end();

		tinyxml2::XMLElement* vertexbufferElem = pXMLDocument->NewElement("vertexbuffer");
		submeshElem->InsertEndChild(vertexbufferElem);
		vertexbufferElem->SetAttribute("positions", true);
		vertexbufferElem->SetAttribute("normals", true);

		for (; vertexBeginIte != vertexEndite; ++vertexBeginIte)
		{
			vertexBeginIte->exportGeometryXml(vertexbufferElem, pXMLDocument);
		}
		...
		vertexbufferElem = pXMLDocument->NewElement("vertexbuffer");
		submeshElem->InsertEndChild(vertexbufferElem);
		vertexbufferElem->SetAttribute("texture_coord_dimensions_0", "float2");
		vertexbufferElem->SetAttribute("texture_coords", 1);

		for (; vertexBeginIte != vertexEndite; ++vertexBeginIte)
		{
			vertexBeginIte->exportTextureCoordXml(vertexbufferElem, pXMLDocument);
		}
		...
		tinyxml2::XMLElement* boneassignmentsElem = pXMLDocument->NewElement("boneassignments");
		submeshElem->InsertEndChild(boneassignmentsElem);

		tinyxml2::XMLElement* vertexboneassignmentElem;
		int idx = 0;
		for (; vertexBeginIte != vertexEndite; ++vertexBeginIte)
		{
			vertexboneassignmentElem = pXMLDocument->NewElement("vertexboneassignment");
			vertexboneassignmentElem->SetAttribute("vertexindex", idx);
			vertexBeginIte->exportVertexBoneAssignmentXml(vertexboneassignmentElem, pXMLDocument);
			++idx;
		}
	}
	...
	bool ExMesh::exportMeshAndSkin2Xml()
	{
		...
		// bones 节点
		tinyxml2::XMLElement* meshElem = pXMLDocument->NewElement("mesh");
		pXMLDocument->InsertEndChild(meshElem);
		// bone 每一个骨头节点
		tinyxml2::XMLElement* submeshesElem = pXMLDocument->NewElement("submeshes");
		meshElem->InsertEndChild(submeshesElem);

		// 导出每一个 submesh 信息
		std::vector<ExSubMesh>::iterator beginIte, endIte;
		beginIte = m_subList.begin();
		endIte = m_subList.end();
