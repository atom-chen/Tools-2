...
{
	void ExBone::exportXml(tinyxml2::XMLElement* boneElem, tinyxml2::XMLDocument* pXMLDocument)
	{
		boneElem->SetAttribute("id", id);
		boneElem->SetAttribute("name", name.c_str());

		tinyxml2::XMLElement* posElem;
		tinyxml2::XMLElement* rotElem;
		tinyxml2::XMLElement* axisElem;

		posElem = pXMLDocument->NewElement("position");
		boneElem->InsertEndChild(posElem);

		boneElem->SetAttribute("x", trans.x);
		boneElem->SetAttribute("y", trans.y);
		boneElem->SetAttribute("z", trans.z);

		rotElem = pXMLDocument->NewElement("rotation");
		boneElem->InsertEndChild(rotElem);

		boneElem->SetAttribute("angle", rot.w);

		axisElem = pXMLDocument->NewElement("axis");
	...
	bool ExSkeleton::exportSkeletonAndAnimation2Xml()
	{
		...
		// bones 节点
		tinyxml2::XMLElement* bonesElem = pXMLDocument->NewElement("bones");
		pXMLDocument->InsertEndChild(bonesElem);
		// bone 每一个骨头节点
		tinyxml2::XMLElement* boneElem;

		// 最大骨骼检测
		if (m_joints.size() > MAX_NUM_BONES)
		{
			MessageBox(NULL, _T("Failure: Skeleton has more than OGRE_MAX_NUM_BONES.  No bones will be exported."), _T("EasyOgre Export Error"), MB_OK | MB_ICONWARNING);
			EasyOgreExporterLog("Failure: Skeleton has more than OGRE_MAX_NUM_BONES.  No bones will be exported.\n");
			return false;
		...
		for (size_t i = 0; i < m_joints.size(); i++)
		{
			ExBone j = m_joints[i];		// 获取一个骨头
			boneElem = pXMLDocument->NewElement("bone");
			bonesElem->InsertEndChild(boneElem);
			j.exportXml(boneElem, pXMLDocument);
		}

		tinyxml2::XMLElement* bonehierarchyElem = pXMLDocument->NewElement("bonehierarchy");
		tinyxml2::XMLElement* boneparentElem;
		ExBone curBone;
		ExBone parentBone;
		// Create the hierarchy
		for (size_t i = 0; i < m_joints.size(); i++)
		{
			...
			{
				curBone = m_joints[i];
				parentBone = m_joints[parentIdx];

				boneparentElem = pXMLDocument->NewElement("boneparent");
				bonehierarchyElem->InsertEndChild(boneparentElem);
