...
class LuaExportMessage(object):
    ...
    def exportMessage(fHandle, message):
        # 写入类的名字
        LuaExportMessage.exportClsDeclStart(fHandle, message)
        # 写入类的成员
        LuaExportMessage.exportMemDecl(fHandle, message)
        # 与后面分割一个空格
        AppSysBase.instance().getClsUtils().writeNewLine2File(fHandle)
        # 写入构造函数
        LuaExportMessage.exportConstruct(fHandle, message)
        # 写入序列化函数
        LuaExportMessage.exportSerialize(fHandle, message)
        # 写入反序列化函数
        LuaExportMessage.exportDerialize(fHandle, message)
        # 写入类的右括号
        LuaExportMessage.exportClsDeclEnd(fHandle, message)
        # 输入一个空行，以便隔开
        AppSysBase.instance().getClsUtils().writeNewLine2File(fHandle)
    ...
    def exportSerialize(fHandle, message):
        ...
        for member in message.getMemberList():
            ...
            if member.getPropertyType() == PropertyType.eInt8 or \
                ...
                LuaExportMessage.exportArrSerialize(fHandle, message, member, CSharpPropertyType2PropertyData.m_sType2PropertyData[member.getPropertyType()].m_propertyTypeKeyWord, CSharpPropertyType2PropertyData.m_sType2PropertyData[member.getPropertyType()].m_serializeFuncKeyWord)
                serializeStr = ""
    ...
    def exportDerialize(fHandle, message):
        ...
        for member in message.getMemberList():
            ...
            if member.getPropertyType() == PropertyType.eInt8 or \
                ...
                LuaExportMessage.exportArrDerialize(fHandle, message, member, CSharpPropertyType2PropertyData.m_sType2PropertyData[member.getPropertyType()].m_propertyTypeKeyWord, CSharpPropertyType2PropertyData.m_sType2PropertyData[member.getPropertyType()].m_derializeFuncKeyWord)
                serializeStr = ""
