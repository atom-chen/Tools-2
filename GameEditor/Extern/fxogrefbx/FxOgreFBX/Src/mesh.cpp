...
#else
		Ogre::v1::MeshPtr pOgreMesh = loadOgreMeshFromFile(managers, meshfilepath);
#endif
...
#else
	Ogre::v1::MeshPtr Mesh::loadOgreMeshFromFile(OgreManagers& managers, std::string &meshfilepath)
#endif
    {
        ...
        Ogre::MeshSerializer meshSerializer;
#else
		Ogre::v1::MeshPtr pMesh = managers.meshMgr->createManual("LocalMesh", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
		Ogre::v1::MeshSerializer meshSerializer;
#endif
...
#else
	bool Mesh::replaceBSAnimation(Ogre::v1::MeshPtr pMesh, const std::string& clipName, float start, float stop, float rate, ParamList& params)
#endif
    {
        Animation a;
...
#else
		Ogre::v1::MeshPtr pMesh = Ogre::v1::MeshManager::getSingleton().createManual(m_name.c_str(),
			Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
		Ogre::MeshPtr v2Mesh = Ogre::MeshManager::getSingleton().createManual(
			m_name.c_str(), Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
#endif
...
#else
			v2Mesh->importV1(pMesh.get(), false, false, false);
			serializer.exportMesh(v2Mesh.getPointer(), params.meshFilename.c_str());
#endif
        }
        catch( Ogre::Exception& e )
        {
...
#else
	bool Mesh::createOgreSharedGeometry(Ogre::v1::MeshPtr pMesh, ParamList& params)
#endif
    {
        int i,j;
        bool stat;
...
#else
		Ogre::v1::VertexDeclaration* pOptimalDecl = pDecl->getAutoOrganisedDeclaration(params.exportVBA, params.exportBlendShapes || params.exportVertAnims, false);
#endif
...
#else
			pMesh->_rationaliseBoneAssignments(pMesh->sharedVertexData[0]->vertexCount, vbas);
#endif
...
#else
		pMesh->sharedVertexData[0]->reorganiseBuffers(pOptimalDecl);
#endif
...
#else
	bool Mesh::createOgreVertexBuffer(Ogre::v1::MeshPtr pMesh, Ogre::v1::VertexDeclaration* pDecl, const std::vector<vertex>& vertices)
#endif
    {
#if OGRE_VERSION < 0x020000
		...
		Ogre::v1::HardwareVertexBufferSharedPtr vbuf =
			...
			pMesh->sharedVertexData[0]->vertexCount,
			Ogre::v1::HardwareBuffer::HBU_STATIC_WRITE_ONLY);
		pMesh->sharedVertexData[0]->vertexBufferBinding->setBinding(0, vbuf);
#endif
...
#else
	bool Mesh::createOgrePoses(Ogre::v1::MeshPtr pMesh, ParamList& params)
#endif
    {
        int poseCounter = 0;
        if (params.useSharedGeom)
...
#else
						Ogre::v1::Mesh::PoseIterator pose_it = pMesh->getPoseIterator();
#endif
...
#else
							Ogre::v1::Pose* current_pose = pose_it.getNext();
#endif
...
#else
						Ogre::v1::Pose* pPose = pMesh->createPose(0, p->name.c_str());
#endif
...
#else
						Ogre::v1::Pose* pPose = pMesh->createPose(static_cast<Ogre::ushort>(i + 1), p->name.c_str());
#endif
                        // Set the pose attributes
...
#else
	bool Mesh::createOgreVertexAnimations(Ogre::v1::MeshPtr pMesh, ParamList& params)
#endif
    {
...
#else
			Ogre::v1::Animation* pAnimation = pMesh->createAnimation(m_vertexClips[i].m_name.c_str(), m_vertexClips[i].m_length);
#endif
...
#else
				Ogre::v1::VertexAnimationTrack* pTrack;
#endif
...
#else
					Ogre::v1::VertexMorphKeyFrame* pKeyframe = pTrack->createVertexMorphKeyFrame(t->m_vertexKeyframes[k].time);
#endif
...
#else
	bool Mesh::createOgrePoseAnimation(Ogre::v1::MeshPtr pMesh, Animation &a, ParamList& params)
#endif
    {
...
#else
		Ogre::v1::Animation* pAnimation = pMesh->createAnimation(a.m_name.c_str(), a.m_length);
#endif
...
#else
				Ogre::v1::VertexPoseKeyFrame* pKeyframe = pTrack->createVertexPoseKeyFrame(t->m_vertexKeyframes[k].time);
#endif
...
#else
	bool Mesh::createOgrePoseAnimations(Ogre::v1::MeshPtr pMesh, ParamList& params)
#endif
    {
