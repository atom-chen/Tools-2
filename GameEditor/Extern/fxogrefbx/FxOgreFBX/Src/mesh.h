...
#else
		Ogre::v1::MeshPtr loadOgreMeshFromFile(OgreManagers& managers, std::string &meshFilePath);
#endif
...
#else
		bool replaceBSAnimation(Ogre::v1::MeshPtr pMesh, const std::string& clipName, float start, float stop, float rate, ParamList& params);
#endif
...
#else
		bool createOgreSharedGeometry(Ogre::v1::MeshPtr pMesh,ParamList& params);
#endif
#if OGRE_VERSION < 0x020000
...
#else
		bool createOgreVertexBuffer(Ogre::v1::MeshPtr pMesh, Ogre::v1::VertexDeclaration* pDecl, const std::vector<vertex>& vertices);
		//create Ogre poses for pose animation
		bool createOgrePoses(Ogre::v1::MeshPtr pMesh, ParamList& params);
		//create vertex animations for an Ogre mesh
		bool createOgreVertexAnimations(Ogre::v1::MeshPtr pMesh, ParamList& params);
		//create pose animations for an Ogre mesh
		bool createOgrePoseAnimations(Ogre::v1::MeshPtr pMesh, ParamList& params);
		// Create a single pose animation from an Animation object
		bool createOgrePoseAnimation(Ogre::v1::MeshPtr pMesh, Animation& a, ParamList& params);
#endif
