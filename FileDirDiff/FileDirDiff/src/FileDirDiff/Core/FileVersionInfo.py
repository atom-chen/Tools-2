...
class BuildFileVersion(object):
    def __init__(self):
        self.m_srcVerFileName = AppSysBase.instance().m_config.preVerFilePath()  # 原始version版本信息文件,这个本来打算根据md来计算一个从0开始不断增大的一个整数
        self.m_destVerFileName = AppSysBase.instance().m_config.curVerFilePath()  # 目标version版本信息文件
        
        self.m_srcMdFileName = AppSysBase.instance().m_config.preCKFilePath()  # 原始md版本信息文件
        self.m_destMdFileName = AppSysBase.instance().m_config.curCKFilePath()  # 目标md版本信息文件
    ...
    def buildVersionFile(self):
        AppSysBase.instance().m_logSys.info('start build version file')
        ...
        for veritem in self.m_destVerlist:
            #if AppSys.AppSysBase.instance().curverFileCount > 0:
            if AppSysBase.instance().get_curverFileCount > 0:
                fHandleVerDest.write('\n')
            
            #AppSys.AppSysBase.instance().curverFileCount += 1
            AppSysBase.instance().add_curverFileCount(1)
    ...
    def outVerSwf(self):
        ...
        byteArray.setAttribute('file', AppSysBase.instance().m_config.curCKFilePath())
        byteArray.setAttribute('class', AppSysBase.instance().m_config.allverclass)
    ...
    def outVerAppSwf(self):
        ...
        byteArray = doc.createElement('bytearray')
        byteArray.setAttribute('file', AppSysBase.instance().m_config.appCKFilePath())
