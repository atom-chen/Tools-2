...
class BuildFileVersion(object):
    def __init__(self):
        self.m_srcVerFileName = GlobalData.g_pAppSys.m_config.preVerFilePath()  # 原始version版本信息文件,这个本来打算根据md来计算一个从0开始不断增大的一个整数
        self.m_destVerFileName = GlobalData.g_pAppSys.m_config.curVerFilePath()  # 目标version版本信息文件
        
        self.m_srcMdFileName = GlobalData.g_pAppSys.m_config.preCKFilePath()  # 原始md版本信息文件
        self.m_destMdFileName = GlobalData.g_pAppSys.m_config.curCKFilePath()  # 目标md版本信息文件
    ...
    def buildVersionFile(self):
        GlobalData.g_pAppSys.m_logSys.info('start build version file')
        ...
        for veritem in self.m_destVerlist:
            #if AppSys.GlobalData.g_pAppSys.curverFileCount > 0:
            if GlobalData.g_pAppSys.get_curverFileCount > 0:
                fHandleVerDest.write('\n')
            
            #AppSys.GlobalData.g_pAppSys.curverFileCount += 1
            GlobalData.g_pAppSys.add_curverFileCount(1)
            fHandleVerDest.write(veritem.m_filename + '=' + str(veritem.m_version))
            
        GlobalData.g_pAppSys.m_logSys.info('end build version file')

        # 生成版本文件
    def readPreVersionFile(self):
        GlobalData.g_pAppSys.m_logSys.info('start read preversion')
        
        preMdFileName = GlobalData.g_pAppSys.m_config.preCKFilePath()
        bMdSrcExist = False
        ...
        GlobalData.g_pAppSys.m_logSys.info('end read preversion')
    ...
    def outVerSwf(self):
        ...
        tmpdir = os.path.join(GlobalData.g_pAppSys.m_config.destrootpath, GlobalData.g_pAppSys.m_config.tmpDir)
        ...
        byteArray.setAttribute('file', GlobalData.g_pAppSys.m_config.curCKFilePath())
        byteArray.setAttribute('class', GlobalData.g_pAppSys.m_config.allverclass)
        ...
        open('%s\\%s' % (GlobalData.g_pAppSys.m_config.destrootpath + '/' + GlobalData.g_pAppSys.m_config.outDir, swfName), 'wb').write(open('%s\\library.swf' % (GlobalData.g_pAppSys.m_config.destrootpath + '/' + GlobalData.g_pAppSys.m_config.tmpDir), 'rb').read())
        os.remove('%s\\library.swf' % (GlobalData.g_pAppSys.m_config.destrootpath + '/' + GlobalData.g_pAppSys.m_config.tmpDir))
        os.remove('%s.swc' % xmlFullName)
        GlobalData.g_pAppSys.m_logSys.info(GlobalData.g_pAppSys.m_config.preckallverfilename + '包 %s 完成' % swfName)
    ...
    def outVerAppSwf(self):
        ...
        byteArray = doc.createElement('bytearray')
        byteArray.setAttribute('file', GlobalData.g_pAppSys.m_config.appCKFilePath())
        ...
        os.remove('%s\\library.swf' % (GlobalData.g_pAppSys.m_config.destrootpath + '/' + GlobalData.g_pAppSys.m_config.tmpDir))
        os.remove('%s.swc' % xmlFullName)
        GlobalData.g_pAppSys.m_logSys.info(GlobalData.g_pAppSys.m_config.preckappverfilename + '包 %s 完成' % swfName)
